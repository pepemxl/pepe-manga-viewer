...
def get_series(series_id: int, db: Session = Depends(get_db)):
    ...
    for c in s.chapters:
        ...
        chapters.append({
            ...
            "number": c.number,
            "number_sort": float(c.number_sort) if c.number_sort is not None else 0.0,
            "title": c.title,
    ...
    # newest chapter first by numeric sort key; tiebreaker on the rendered number
    chapters.sort(key=lambda x: (x["number_sort"], x["number"]), reverse=True)
...
@router.post("/{series_id}/redetect-chapters")
def redetect_chapters(series_id: int, db: Session = Depends(get_db)):
    """Re-parse chapter source paths with the current chapter-number detector
    and update number / number_sort / title in place. Sort order follows."""
    s = db.query(models.Series).get(series_id)
    if not s:
        raise HTTPException(404, "series not found")
    return {"ok": True, **scanner.redetect_series_chapter_numbers(db, series_id)}
