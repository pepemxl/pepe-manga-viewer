...
- each `.cbz` / `.zip` / `.cbr` / `.rar` archive, `.pdf` book, and image sub-folder is a chapter;
...
fully offline. PDFs are rasterized with **pdf.js** in the webview; CBR/RAR via the
bundled `unrar` library — both without any external binaries. The library index
lives in the webview's `localStorage`; **Remove** deletes both the index entry
and the extracted files.

Supported: **CBZ/ZIP**, **CBR/RAR**, **PDF**, and **image folders**.
